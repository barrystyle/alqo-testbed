...
{
    const char* pszTimestamp = "ALQO - concept of merged utxo/state-like system";
...
public:
    CMainParams() {
        ...
        genesis = CreateGenesisBlock(1231006505, 2083236893, 0x1d00ffff, 1, 50 * COIN);  // set first so we arent using uninit genesis var

        uint32_t nTime = 1572654719;
        uint32_t nNonce = 0;
        while (UintToArith256(genesis.GetHash()) >
               UintToArith256(consensus.powLimit))
        {
               nNonce++;
               genesis = CreateGenesisBlock(nTime, nNonce, 0x1d00ffff, 1, 0 * COIN);
               if (nNonce % 1024 == 0) printf("\rnonce %08x");
        }
        printf("genesis was %s\n", genesis.ToString().c_str());
